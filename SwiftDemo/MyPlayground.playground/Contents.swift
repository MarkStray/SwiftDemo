...
func _swapTwoInts(inout a: Int, inout b: Int) {
    let temporaryA = a
    a=b
    b = temporaryA
}
var _someInt = 3
var _anotherInt = 107
_swapTwoInts(&_someInt, b: &_anotherInt)
print("someInt is now \(_someInt), and anotherInt is now \(_anotherInt )")
...
/*注意: 在所有三个函数中,a 和 b 的类型是一样的。如果 a 和 b 不是相同的类型,那它们 俩就不能互换值。Swift 是类型安全的语言,所以它不允许一个 String 类型的变量和一个
 Double 类型的变量互相交换值*/
...
//泛型函数
/*尖括号告诉 Swift 那个 T 是 swapTwoValues 函数所定义的一个类型。因为
...
//------ 2.3
//http://www.cocoachina.com/swift/20160219/15334.html
//在swift中很多关键字比如in,repeat,defer都不能用做参数名
//增加#if swift 语法判断当前swift版本
//关联已存在类型时，不再使用typealias
//2.2中将第一种情况时，启用新的关键字associatedtype
//一个新的方法生成selector
//异常处理 http://www.cocoachina.com/swift/20160125/14915.html
